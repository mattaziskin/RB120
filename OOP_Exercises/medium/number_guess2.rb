...
In the previous exercise, you wrote a number guessing game that determines a s
...
class GuessingGame
  attr_accessor :secret_number, :low_limit, :high_limit, :range, :guesses
  ...
  def play_game
    result = nil
    self.guesses.downto(1) do |remaining_guesses|
      display_guesses_remaining(remaining_guesses)
      result = check_guess(obtain_one_guess)
      puts RESULT_OF_GUESS_MESSAGE[result]
      break if result == :match
